...
mtcars <- read_csv("C:/Users/stevesb/Documents/R/win-library/4.0/readr/extdata/mtcars.csv")

```
You can also just specify file location by wrapping read_csv around readr_example()
...
```{r}
WSA_dat<-read.csv("data/WSA_data.csv")

# read.csv time
system.time(read.csv("data/WSA_data.csv"))

# read_csv time
system.time(read_csv("data/WSA_data.csv"))
...
# see if any ODBC DSN's have been set up on computer
odbcListDataSources()

# create a connection to a DSN.  This is set up ahead of time on your computer.
con <- dbConnect(odbc(), "GlobalInvHist_be")


# create a connection without a DSN

con <- dbConnect(odbc::odbc(), 
                 Driver = "MySQL ODBC 8.0 Unicode Driver",
                 Host = "localhost",
                 Database = "globalinvhist_be",
                 UID = "username",
                 PWD = "password")
```
...
In newer versions of RStudio, ther is a 'connections' tab in the upper righthand panel. This will let you explore databases, tables, columns of your database connection.  Alternatively, you can access that information with in the console.
...
dbListFields(con, "taxanew")

# return a whole table
taxanew <- dbReadTable(con, "taxanew")
...
2.   Submit both your dataset (or small part of it) and an R script of the code you used to import it onto d2l
